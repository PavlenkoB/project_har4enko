    ...
    private void from_lay_load() {
        ...
        try {
            if(((id_Lable)CB_from_arch.getSelectionModel().getSelectedItem()).getDbid().intValue()!=-1) {
                ...
                rs = DB_connection.executeQuery("SELECT * FROM PATERNS WHERE TYPE='GLOBAL' ORDER BY NAME ASC");
                ObservableList<id_Lable> items = FXCollections.observableArrayList();
                while (rs.next()) {
                    id_Lable tmp_lable = new id_Lable(rs.getInt("ID"), rs.getString("NAME"));
                    items.add(tmp_lable);
                ...
                CB_from_mod.setItems(FXCollections.observableArrayList());
                CB_from_lay.setItems(FXCollections.observableArrayList());
    ...
    private void from_mod_load() {
        ...
        try {
            ...
            if (((id_Lable)CB_from_lay.getSelectionModel().getSelectedItem()).getDbid().intValue()!=-1) {
                ...
                //непривязані патерни
                rs = DB_connection.executeQuery("SELECT * FROM PATERNS WHERE MOD_ID=-1 AND ARCH_ID="+((id_Lable) CB_from_arch.getSelectionModel().getSelectedItem()).getDbid().intValue()+" ORDER BY NAME ASC");
                ObservableList<id_Lable> items = FXCollections.observableArrayList();
                while (rs.next()) {
                    id_Lable tmp_lable = new id_Lable(rs.getInt("ID"), rs.getString("NAME"));
                    items.add(tmp_lable);
