...
public class CreateArchController implements Initializable {
    ...
    public void close(ActionEvent actionEvent) throws IOException {
        try {
            ...
            Parent root;
            root = FXMLLoader.load(getClass().getResource("/views/create_arch/Close.fxml"));
            ...
            while (rs_lay.next()) {
                ...
                while (rs_mod.next()) {
                    ...
                    while (rs_pat.next()) {//Все патерны что подходят модулю в кнопку
                        arc_choise.getLayers().get(s_lay).getModules().get(s_mod).getAvilablePatterns().add(new Pattern(rs_pat.getInt("ID"), rs_pat.getInt("MOD_ID"), rs_pat.getString("NAME"), rs_pat.getString("DESCRIPTION"), rs_pat.getString("VALUE")));
        ...
        for (int i = 0; i < sel_pat_splitmenu.size(); i++) {
            ...
            if (all) {
                ...
                try {
                    ...
                    rs_lay = derby_DB.executeQuery("SELECT * FROM LAYER WHERE ID=" + modules.getLayId());
                    rs_lay.next();
                    ...
                    if (arc_choise.getLayers().get(l).getId() == layer.getId()) {
                        for (int k = 0; k < arc_choise.getLayers().get(l).getModules().size(); k++) {
                            if (arc_choise.getLayers().get(l).getModules().get(k).getId() == modules.getId()) {
                                module_done.get(i).getAvilablePatterns().addAll(arc_choise.getLayers().get(l).getModules().get(k).getAvilablePatterns());
                            }
                        }
                    }
                }
                module_done.get(i).setId(modules.getId());
                module_done.get(i).setLayId(modules.getLayId());
                ...
                for (int j = 0; j < sel_pat_splitmenu.get(i).getItems().size(); j++) {
                    if (((CheckMenuItem) sel_pat_splitmenu.get(i).getItems().get(j)).selectedProperty().getValue() & !sel_pat_splitmenu.get(i).getItems().get(j).getText().equals("Обрати усі")) {
                        ...
                        module_done.get(i).getAvilablePatterns().add(paterns);
                        module_done.get(i).setId(modules.getId());
                        module_done.get(i).setLayId(modules.getLayId());
                        module_done.get(i).setName(modules.getName());
                    }
...
/**
 ...
 module_done.get(i).getAvilablePatterns().addAll(arc_choise.getLayers().get(j).getModules().get(k).getAvilablePatterns());
 }
 }
 }
 }
 ...
 module_done.get(i).setName(modules.getName());
        ...
        for (int i = 0; i < module_done.size(); i++) {
            for (int j = 0; j < module_done.get(i).getAvilablePatterns().size(); j++) {
                ...
                modules_done.get(num_mod_done).setLayId(module_done.get(i).getLayId());
                modules_done.get(num_mod_done).getAvilablePatterns().add(module_done.get(i).getAvilablePatterns().get(j));
                modules_done.get(num_mod_done).setSelectedPattern(module_done.get(i).getAvilablePatterns().get(j));
                num_mod_done++;
            }
        }
        ...
        for (int i = 0; i < arc_choise.getLayers().size(); i++) {
            ...
            for (int j = 0; j < arc_choise.getLayers().get(i).getModules().size(); j++) {
                ...
                gridPanes_pat.get(i).get(j).setVgap(arc_choise.getLayers().get(i).getModules().get(j).getAvilablePatterns().size());
                int com = 0;
                for (int k = 0; k < module_done.size(); k++) {
                    if (module_done.get(k).getId() == arc_choise.getLayers().get(i).getModules().get(j).getId()) {
                        for (int m = 0; m < module_done.get(k).getAvilablePatterns().size(); m++) {

                            tmp_text_area = new TextArea(module_done.get(k).getAvilablePatterns().get(m).getDescription());
                            ...
                            tmp_lable = new Label(module_done.get(k).getAvilablePatterns().get(m).getName());
                            tmp_lable.setMinWidth(300);
                            tmp_lable.setMaxWidth(300);
                            gridPanes_pat.get(i).get(j).add(tmp_lable, 0, com);
                            //tmp_lable = new Label(module_done.get(k).getAvilablePatterns().get(m).getDescription());
        ...
        for (int i = 0; i < arc_choise.getLayers().size(); i++) {
            for (int j = 0; j < module_done.size(); j++) {
                if (modules_done.get(j).getLayId() == arc_choise.getLayers().get(i).getId()) {
